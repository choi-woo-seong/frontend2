...
import { Await, useLocation, useNavigate, useSearchParams } from 'react-router-dom';
...
import { FaSearch, FaRegHeart, FaHeart, FaMapMarkerAlt, FaChevronLeft } from 'react-icons/fa';
...
const convertToDatabaseGrade = (inputGrade) => {
  const map = {
    '1등급': '1',
    '2등급': '2',
    '3등급': '3',
  ...
  return map[inputGrade] || inputGrade;
};
...
function SearchPage() {
  ...
  useEffect(() => {
    fetchFacilities();
  }, [category, selectedRegion, selectedFacilityType, selectedEvaluationGrade, selectedSort]);
  ...
  const fetchFacilities = async () => {
    ...
    try {
      ...
      if (selectedFacilityType !== '시설규모') {
        queryParams.append('facilitySize', facilitySizeMap[selectedFacilityType]);
      }
      ...
      if (selectedEvaluationGrade !== '평가등급') {
        ...
        if (category === '요양병원') {
          gradeToSend = convertToDatabaseGrade(selectedEvaluationGrade); // 🛠
        } else {
          gradeToSend = selectedEvaluationGrade;
        }

        queryParams.append('grade', gradeToSend);
      }
  ...
  const filtered = facilities.filter((f) => {
    if (searchKeyword && !f.name.includes(searchKeyword) && !f.address.includes(searchKeyword)) {
      return false;
    }

    if (selectedFacilityType !== '시설규모' && f.facilitySize !== facilitySizeMap[selectedFacilityType]) {
      return false;
    }

    if (
      selectedEvaluationGrade !== '평가등급' &&
      ((category === '요양병원' && convertToDatabaseGrade(selectedEvaluationGrade) !== f.grade) ||
        (category !== '요양병원' && convertGradeToLetter(f.grade).toUpperCase() !== selectedEvaluationGrade.toUpperCase()))
    ) {
      return false;
    }
  ...
  return (
    <div className='searchpage-container'>
      ...
      <div className='searchpage-region'>
        <button onClick={() => setRegionModalOpen(true)} className='w-full flex items-center justify-between bg-gray-100 px-4 py-3 rounded-md text-sm'>
      ...
      <div className='searchpage-search-wrapper'>
        <div className='searchpage-search'>
          ...
          <input type='text' placeholder='검색어 입력' className='search-input' value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} />
        </div>
      </div>
      ...
      <div className='searchpage-facility-list'>
        <ul className='facility-list'>
          ...
          {!loading &&
            !error &&
            filtered.map((fac) => (
              <li key={fac.id} className='facility-item' onClick={() => handleGoToDetail(fac.id)} style={{ cursor: 'pointer' }}>
                <div className='facility-info'>
                  <div className='facility-text'>
                    <h3>{fac.name}</h3>
                    <p>{fac.address}</p>
                    {fac.grade && <span className='facility-tag grade-tag'>등급: {convertGradeToLetter(fac.grade)}</span>}

                    {fac.facilitySize && <span className='facility-tag size-tag'>규모: {fac.facilitySize}</span>}
                    {fac.establishedYear && <span className='facility-tag'>설립 {new Date().getFullYear() - fac.establishedYear}년</span>}
                  </div>
                  <div className='facility-image-container'>
                    ...
                    >
                      {likedFacilities.includes(fac.id) ? <FaHeart className='liked' /> : <FaRegHeart />}
                    </button>
                  </div>
                </div>
              </li>
      ...
      {/* 모달 */}
      <RegionSelectorModal isOpen={regionModalOpen} onClose={() => setRegionModalOpen(false)} onSelectRegion={setSelectedRegion} />
      ...
      {evaluationGradeModalOpen && category !== '실버타운' && (
        <FilterModal
          title='평가등급'
          options={category === '요양병원' ? ['1등급', '2등급', '3등급', '4등급', '5등급', '등급제외'] : ['A', 'B', 'C', 'D', 'E', '등급제외']}
